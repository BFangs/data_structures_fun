...
class Queue(object):
    ...
    def __repr__(self):
        if not self._head:
            ...
            return "<Queue head=%s tail=%s length=%d>" % (
                self._head.data, self._tail.data, self._length)
...
class ArrayQueue(object):
    ...
    def __repr__(self):
        return "<ArrayQueue head=%s length=%d>" % (
            self._data[self._front], self._length)
...
if __name__ == '__main__':
    ...
    print timeit("q.dequeue(); q.enqueue(0)", "from queue import Queue; q=Queue(range(50000))")

    print "Time for our ArrayQueue class"
    print timeit("q.dequeue(); q.enqueue(0)", "from queue import ArrayQueue; q=ArrayQueue(range(50000))")

    print "Time for native Python list as queue"
    print timeit("q.append(1); q.pop(0)", "q=range(50000)")

    print "Time for native Python deque as queue"
    print timeit("q.append(1); q.popleft()", "from collections import deque; q=deque(range(50000))")
